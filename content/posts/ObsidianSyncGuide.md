...
> 📚 本文是「Obsidian 知识管理」系列第二篇。同系列：
> - [从印象笔记到 Obsidian：一次完整的笔记迁移实战](/posts/evernotetoobsidian/)
> - [Obsidian 三设备同步实战：Windows 双机 Git + Android Syncthing](/posts/obsidianthreedevicesync/)
...
git clone git@github.com:<你的用户名>/obsidian-<vault名>.git <你的本地笔记目录>
```
...
打开 Obsidian → 打开其他仓库 → 打开文件夹作为仓库 → 选择 `<你的本地笔记目录>`
...
```bash
cd <你的本地笔记目录>
git remote set-url origin git@github.com:<你的用户名>/obsidian-<vault名>.git
```
...
```bash
git remote -v
# origin  git@github.com:<你的用户名>/obsidian-<vault名>.git (fetch)
# origin  git@github.com:<你的用户名>/obsidian-<vault名>.git (push)
```
...
```bash
# 备份当前目录
move <你的本地笔记目录> <你的本地笔记目录>_backup
# 重新克隆
git clone git@github.com:<你的用户名>/obsidian-<vault名>.git <你的本地笔记目录>
```
