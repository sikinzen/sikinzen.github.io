...
```
成功派生 18 个数据库的加密密钥
  MicroMsg.db: 39.2MB, key=xxxx...
  MSG0.db: 156.7MB, key=xxxx...
  ...
```
...
```bash
# 按昵称查看
wechat-cli history "联系人A" --limit 20 --format text

# 按微信 ID 查看（如果昵称不匹配，用 ID 更准确）
wechat-cli history "<联系人微信ID>" --limit 20 --format text
```

### 按时间范围查询

```bash
wechat-cli history "<联系人微信ID>" --start-time "2026-06-05" --end-time "2026-06-06" --format text
```
...
# 在指定聊天中搜索
wechat-cli search "需求" --chat "<聊天名称>" --format text
```
...
```bash
# 导出为 Markdown
wechat-cli export "联系人A" --format markdown

# 导出为 HTML
wechat-cli export-html "联系人A"
```
...
```python
# 直接把 wx_key 输出当密钥用
key = bytes.fromhex("a1b2c3d4e5f6...")
# → SQLCipher 解密失败！
```
...
passphrase = bytes.fromhex("a1b2c3d4e5f6...")  # wx_key 提取的值
...
6. **隐私保护** — 发布相关内容时，务必将联系人姓名、微信 ID、passphrase 等敏感信息做脱敏处理

**建议：**
- 仅在**个人学习研究**目的下使用
- 不要在**处理重要业务的账号**上使用
- 定期备份 `~/.wechat-cli/all_keys.json`，避免重复操作
- **发布博客或文章时，务必对 passphrase、联系人姓名、微信 ID 等做脱敏处理**
- 了解并接受可能的后果
